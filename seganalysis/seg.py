...
def segrun(mt, ped, outfolder,hl, *vcffile):
    """
    ...
    if vcffile is not None and len(vcffile) >= 1 :
        ...
        if len(pd.unique(ped.loc[:,'phenotype']))==1:
            segrun_family_wise_CSQ_one(mt, ped, outfolder,hl,csqlabel)
        else:    
            segrun_family_wise_CSQ(mt, ped, outfolder,hl,csqlabel)
    else:
...
def segrun_family_wise_CSQ_one(mt, ped, outfolder,hl,csqlabel):
    # print("Run segregation on CSV with CSQ")
    ### use CSQ   
    mt1=mt
...
##### seg with CSQ
def segrun_family_wise_CSQ(mt, ped, outfolder,hl,csqlabel):
    # print("Run segregation on CSV with CSQ")
    ### use CSQ   
    mt1=mt
    mt1=mt1.annotate_rows(kept_transcripts =mt1.info.CSQ.map(lambda x: x.split('\|')))
